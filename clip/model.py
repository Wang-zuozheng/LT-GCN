...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = x.reshape(x.shape[0], x.shape[1],
                      -1)  # shape = [*, width, grid ** 2] bs 768 196
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width] bs 196 768
        x = torch.cat([
            self.class_embedding.to(x.dtype) + torch.zeros(
                x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x
        ],
                      dim=1)  # shape = [*, grid ** 2 + 1, width] # noqa  bs 197 768
        x = x + self.positional_embedding.to(x.dtype) # bs 197 768
...
class ModifiedResNet_conv_proj(nn.Module):
    ...
    def forward(self, x):
        ...
        x, attn_weights = self.attnpool(x)# 8 512 50
    ...
    for k, v in old_state_dict.items():
        if k in state_dict.keys():
            if 'visual' in k and ('v_proj' in k or 'c_proj' in k):
                ...
                w_spacial = state_dict[k][1:].permute(1, 0).reshape(1, -1, 7, 7) # 1 2048 7 7
